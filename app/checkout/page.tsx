...
<div className="md:hidden min-h-screen bg-[#f5f5f5] pb-32">
  ...
  <div className="px-2 pt-2 space-y-2">
    ...
    {!isBusinessOrder && (() => {
      ...
      return (
        <section className="bg-white rounded-2xl overflow-hidden" style={{ boxShadow: "0 1px 6px rgba(0,0,0,0.07)" }}>
          {/* Header */}
          <div className="px-4 pt-4 pb-1">
            <div className="text-[13px] font-semibold text-slate-500">Payment</div>
            <div className="text-[11px] text-slate-400 mt-0.5">Select a payment method before placing order</div>
          </div>
          ...
          <div className="mt-2 divide-y divide-slate-100">
            {allVisible.map((opt) => {
              ...
              const isSelectable = !isLocked && !isComingSoon;

              return (
                <button
                  key={opt.id}
                  type="button"
                  onClick={() => { if (isSelectable) setPaymentMethod(opt.id); }}
                  disabled={isLocked}
                  className={`w-full flex items-center gap-3 px-4 py-4 text-left transition-colors ${
                    selected ? "bg-slate-50/80" : isLocked ? "opacity-35 cursor-not-allowed" : "active:bg-slate-50"
                  }`}
                >
                  {/* Icon */}
                  <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${
                    opt.id === "pay_on_delivery" ? "bg-emerald-50" :
                    opt.id === "stripe_card"     ? "bg-blue-50"    :
                    isComingSoon                  ? "bg-amber-50"   : "bg-slate-100"
                  }`}>
                    <Icon className={`w-5 h-5 ${
                      opt.id === "pay_on_delivery" ? "text-emerald-600" :
                      opt.id === "stripe_card"     ? "text-blue-600"    :
                      isComingSoon                  ? "text-amber-500"   : "text-slate-400"
                    }`} />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-[14px] font-bold text-slate-900 leading-snug">{opt.label}</span>
                      {isComingSoon && (
                        <span className="px-1.5 py-0.5 rounded-full text-[8px] font-bold bg-amber-100 text-amber-600 shrink-0">Soon</span>
                      )}
                    </div>
                    ...
                    {opt.id === "stripe_card" ? (
                      <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded border border-slate-200 text-[9px] font-black text-blue-700 tracking-wide bg-white">VISA</span>
                        <span className="inline-flex items-center px-1 py-0.5 rounded border border-slate-200 bg-white gap-0">
                          <span className="w-3 h-3 rounded-full bg-red-500 inline-block" />
                          <span className="w-3 h-3 rounded-full bg-amber-400 inline-block -ml-1.5" />
                        </span>
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded border border-blue-300 bg-blue-600 text-[8px] font-black text-white tracking-wide">AMEX</span>
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded border border-orange-200 text-[8px] font-bold text-orange-500 bg-white">Discover</span>
                      </div>
                    ) : (
                      <div className="text-[11px] text-slate-400 mt-0.5">{opt.badge}</div>
                    )}
                  </div>
          ...
          {otherOpts.length > 0 && (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 py-3 border-t border-slate-100 text-[11px] font-semibold text-slate-400 active:text-slate-600 transition-colors"
            >
          ...
          {/* Coming soon notice — only shown if one is selected */}
        ...
        <div className="max-w-5xl mx-auto px-4 py-4">
          ...
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <button onClick={() => router.push("/shop")} className="hover:text-slate-900 transition-colors">Shop</button>
                ...
                {isBusinessOrder ? (
                  ...
                  <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        return (
                          ...
                          >
                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${
                              ...
                              <Icon className={`w-3.5 h-3.5 ${selected ? "text-white" : isComingSoon ? "text-amber-600" : "text-slate-500"}`} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5">
                                <span className="font-semibold text-slate-900 text-xs">{opt.label}</span>
                                {isComingSoon && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-100 text-amber-700">Soon</span>}
                                {isLocked && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-200 text-slate-500">Locked</span>}
                              </div>
                              <div className={`text-[10px] mt-0.5 ${isLocked ? "text-slate-400" : "text-slate-500"}`}>{opt.badge}</div>
                            </div>
                    ...
                    <button
                      type="button"
                      onClick={() => setPaymentExpanded((v) => !v)}
                      className="w-full flex items-center justify-center gap-1 py-2 border-t border-slate-100 text-[11px] font-semibold text-slate-400 hover:text-slate-600 transition-colors"
                    >
                      {paymentExpanded ? "Show less" : "Show more payment methods"}
                      <ChevronDown className={`w-3 h-3 transition-transform duration-200 ${paymentExpanded ? "rotate-180" : ""}`} />
                    </button>
                    {(paymentMethod === "ceb_link" || paymentMethod === "telebirr") && (
                      <div className="px-4 pb-2.5">
                        <div className="p-2 rounded-lg bg-amber-50 border border-amber-200 text-[11px] text-amber-800 flex items-center gap-2">
                          <Clock className="w-3 h-3 shrink-0" />Coming soon — please use Pay on Delivery.
                        </div>
                      </div>
                    )}
                  </section>
